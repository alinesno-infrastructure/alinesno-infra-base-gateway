...
import com.alinesno.infra.base.gateway.facade.bean.*;
...
import com.alinesno.infra.base.gateway.domain.service.AccountService;
...
import com.alinesno.infra.base.gateway.rest.aop.DataFilterRole;
...
public class RouteRest extends BaseRest<Route> {
    ...
    @DataFilter(type = DataFilterRole.ORGANIZATION)
    ...
    protected Route toRoute(RouteReq routeReq){
        ...
        //添加鉴权器
        if (access != null) {
            List<String> routeAccessList = getRouteAccessList(access);
            route.setFilterAuthorizeName(StringUtils.join(routeAccessList.toArray(), Constants.SEPARATOR_SIGN));
        }

        return toExampleOrg(route, routeReq.getOrgId());
    }

    @NotNull
    private static List<String> getRouteAccessList(RouteAccessBean access) {
