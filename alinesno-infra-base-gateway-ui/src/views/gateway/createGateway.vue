				...
				<el-card shadow="never" class="box-card">
					<div class="clearfix" style="margin-bottom:10px">
						...
						<el-popover trigger="click" placement="right" :width="700">
							<div style="font-size: 10pt;">
								...
								<span>6.示例：</span><br />
								<span>&nbsp;&nbsp;a.服务URL示例：http://server:port、http://server.com、lb://xxx
									,支持但不推荐：http://server:port/api.do。</span><br />
								...
								<span>&nbsp;&nbsp;f.断言Header示例：Header=X-Request-Id,
									\d+，其中\d+表示正则匹配的任意值，需满足java正则表达示或参见官方。</span><br />
								<span>7.如配置多个断言项，则gateway网关采用断言组合匹配方式（and关系）转发到符合的路由服务中。</span>
							</div>
							<template #reference>
								<el-button style="padding: 3px 0;margin-left:5px " type="text" title="说明"><i
										class="fa-solid fa-tags"></i></el-button>
							</template>
						</el-popover>
					...
					<el-row>
						<el-col :span="24">
							<div style="float: left;">
								<el-input size="large" placeholder="示例：project-method" v-model="form.routeId"
									style="width: 343px;" :disabled="idDisabled">
									<template #prepend>RouteId</template>
								</el-input>
							</div>
							<div style="float: left; margin-left: 10px;">
								<el-input size="large" placeholder="示例：CRM" v-model="form.systemCode"
									style="width: 343px;">
									<template #prepend>系统代号</template>
								</el-input>
							</div>
							<div style="float: left; margin-left: 10px;">
								<el-input size="large" placeholder="示例：CRM-用户信息获取网关" v-model="form.name"
									style="width: 343px;">
							...
							<div style="float: left; margin-left: 10px;">
								<el-popover placement="bottom" width="170" trigger="click">
									...
									<template #reference>
										<el-button size="large">服务状态:{{ form.status === '0' ? '启用' : '禁用' }} <el-icon>
												<CaretBottom />
											</el-icon></el-button>
									</template>
								</el-popover>
							...
							<div style="float: left; margin-left: 10px;">
								<el-popover placement="bottom" width="500" trigger="click">
									<el-radio-group v-model="form.groupCode" @change="handleSelectedGroup">
										<el-radio-button v-for="item in groupOptions" :key="item.value"
											:label="item.value">{{ item.label }}</el-radio-button>
									</el-radio-group>
									<template #reference>
										<el-button size="large">分组:{{ groupName }} <el-icon>
												<CaretBottom />
											</el-icon></el-button>
									</template>
								</el-popover>
					...
					<el-row style="margin-top: 20px;">
						...
						<el-col :span="24">
							<div style="float: left;">
								<el-input size="large"
									placeholder="请输入网关服务URL,示例:http://server:port、http://server.com、lb://xxx"
									v-model="form.uri" class="input-with-select" style="width: 696px;">
									<template #prepend>
										<el-select size="large" v-model="form.method" placeholder="请选择"
											style="width: 90px;">
							...
							<div style="float: left; margin-left: 10px;">
								<el-popover placement="bottom" width="500" trigger="click">
									<el-input size="large" placeholder="示例：Path=/route/producer/** 或 /producer/api"
										v-model="form.path" style="width: 475px;">
										<template #prepend>Path=</template>
									</el-input>
									<template #reference>
										<el-button size="large">断言Path={{ form.path }} <el-icon>
												<CaretBottom />
											</el-icon></el-button>
									</template>
								</el-popover>
							...
							<div style="float: left; margin-left: 10px;">
								<el-popover placement="bottom" width="500" trigger="click">
									<el-input size="large" placeholder="示例：Header=X-Request-Id, \d+"
										v-model="form.header">
										<template #prepend>Header=</template>
									</el-input>
									<template #reference>
										<el-button size="large">断言Header={{ form.header }} <el-icon>
												<CaretBottom />
											</el-icon></el-button>
									</template>
								</el-popover>
							...
							<div style="float: left; margin-left: 10px;">
								<el-popover placement="bottom" width="350" trigger="click">
									<el-input size="large" type="number" placeholder="请输入数字，示例：1"
										v-model="form.stripPrefix">
										<template #prepend>StripPrefix=</template>
									</el-input>
									<template #reference>
										<el-button size="large">过滤StripPrefix={{ form.stripPrefix }} <el-icon>
												<CaretBottom />
											</el-icon></el-button>
									</template>
								</el-popover>
						...
						<el-col :span="24">
							<div style="float: left;">
								<el-popover placement="bottom" width="500" trigger="click">
									...
									<template #reference>
										<el-button size="large">断言Host={{ form.host }} <el-icon>
												<CaretBottom />
											</el-icon></el-button>
									</template>
								</el-popover>
							...
							<div style="float: left; margin-left: 10px;">
								<el-popover placement="bottom" width="500" trigger="click">
									<el-input size="large" placeholder="示例：RemoteAddr=192.168.1.1/100"
										v-model="form.remoteAddr">
										<template #prepend>RemoteAddr=</template>
									</el-input>
									<template #reference>
										<el-button size="large">断言RemoteAddr={{ form.remoteAddr }} <el-icon>
												<CaretBottom />
											</el-icon></el-button>
									</template>
								</el-popover>
							...
							<div style="float: left; margin-left: 10px;">
								<el-popover placement="bottom" width="550" trigger="click">
									<el-input size="large" placeholder="示例：RequestParameter=version,v01"
										v-model="form.requestParameter">
										<template #prepend>RequestParameter=</template>
									</el-input>
									<template #reference>
										<el-button size="large">参数RequestParameter={{ form.requestParameter }} <el-icon>
												<CaretBottom />
											</el-icon></el-button>
									</template>
								</el-popover>
							...
							<div style="float: left; margin-left: 10px;">
								<el-popover placement="bottom" width="600" trigger="click">
									<el-input size="large"
										placeholder="示例：RewritePath=/foo/(?<segment>.*), /$\{segment}"
										v-model="form.rewritePath">
										<template #prepend>RewritePath=</template>
									</el-input>
									<template #reference>
										<el-button size="large">重定向RewritePath={{ form.rewritePath }} <el-icon>
												<CaretBottom />
											</el-icon></el-button>
									</template>
								</el-popover>
			...
			<el-col :span="6">
				<el-card shadow="never" class="box-card">
					...
					<el-collapse accordion>
						<el-collapse-item>
							<template #title>
								全局方法&nbsp;&nbsp;<i v-show="hystrix.defaultChecked" class="header-icon el-icon-success"
									style="color: #34bfa3; font-size: 12pt;"></i>
							</template>
							<div><el-checkbox v-model="hystrix.defaultChecked"
									@change="handleHystrixChecked('default')">启用</el-checkbox></div>
							<div>启用超时检测，当请求到响应返回的时间，超过5000ms后，将触发全局Hystrix熔断方法的（name: fallbackcmd），默认回调路径fallbackUri:
								forward:/fallback。</div>
						</el-collapse-item>
						<el-collapse-item title="自定义方法">
							<template #title>
								自定义方法&nbsp;&nbsp;<i v-show="hystrix.customChecked" class="header-icon el-icon-success"
									style="color: #34bfa3; font-size: 12pt;"></i>
							</template>
							<div><el-checkbox v-model="hystrix.customChecked"
									@change="handleHystrixChecked('custom')">启用</el-checkbox></div>
							<div>启用超时检测，当请求到响应返回的时间，超过自定义时长后，将触发自定义Hystrix熔断方法（name:
								customHystrix_no），默认回调路径fallbackUri:
								forward:/fallback/custom 。(如无特殊要求，<span
									style="font-weight: bold;">强列建议用默认全局Hystrix方法</span>，过多定制Hystrix否则会增加gateway网关服务性能压力)
							</div>
							<div style="margin-top: 10px;">
								<el-input size="large" placeholder="触发fallbackcmd超时时长,如:3000"
									v-model="form.fallbackTimeout">
									<template #prepend>超时时长</template>
								</el-input>
							</div>
							<div style="margin-top: 10px;">
								<el-input size="large" placeholder="触发fallbackcmd提示内容,如:system is error"
									v-model="form.fallbackMsg">
					...
					<div class="clearfix">
						...
						<el-popover trigger="click" placement="bottom">
							<div style="font-size: 10pt;">
								...
								<span>3.心跳检测请求Header中带<span
										style="font-weight: bold;">Keepalive:flying-fish-gateway</span>，服务可做特殊性响应。</span><br />
							...
							<template #reference>
								<el-button style="float: right; padding: 3px 0; " icon="Question" type="text"
									title="说明">说明</el-button>
							</template>
						</el-popover>
					</div>
					<el-collapse accordion>
						<el-collapse-item>
							...
							<div style="margin-top: 10px;">
								<el-popover placement="bottom" width="170" trigger="click">
									...
									<template #reference>
										<el-button size="large">告警重试：{{ form.monitor.recover === '0' ? '启用' : '禁用' }}
											<el-icon>
												<CaretBottom />
											</el-icon></el-button>
									</template>
								...
								<el-popover placement="bottom" width="460" trigger="click">
									<el-radio-group v-model="form.monitor.frequency"
										@change="handleSelectedMonitorFrequency">
										<el-radio-button v-for="item in monitorOptions" :key="item.value"
											:label="item.value">{{ item.label }}</el-radio-button>
									</el-radio-group>
									<template #reference>
										<el-button size="large">告警通知频率：{{ monitorFrequencyName }} <el-icon>
												<CaretBottom />
											</el-icon></el-button>
									</template>
								</el-popover>
							...
							<div style="margin-top: 10px;">
								<el-input size="large" placeholder="示例：user1@qq.com,user2@qq.com"
									v-model="form.monitor.emails" maxlength="200" show-word-limit>
									<template #prepend>通知邮箱</template>
								</el-input>
							</div>
							<div style="margin-top: 10px;">
								<el-input size="large" placeholder="示例：XXX网关服务发生告警，请及时处理" v-model="form.monitor.topic"
					...
					<div class="clearfix">
						...
						<el-popover trigger="click" placement="bottom">
							<div style="font-size: 10pt;">
								...
								<span>3.TOKEN过滤目前只对请求Header中带<span
										style="font-weight: bold;">TOKEN</span>做非空验证，暂无其它响应。</span><br />
								<span>4.ID过滤对请求Header中带<span
										style="font-weight: bold;">CLIENTID</span>做较验，非注册客户端ID不可访问。</span><br />
							</div>
							<template #reference>
								<el-button style="float: right; padding: 3px 0; " icon="Question" type="text"
									title="说明">说明</el-button>
				...
				<el-card shadow="never" class="box-card">
					...
					<div style="margin-top: 10px;">
						<el-row :gutter="24">
							...
							<el-col :span="18">
								<el-input size="large" type="number" v-model="form.burstCapacity" :step="20" :min="0"
									:max="10000" style="width: 60%;" />
							</el-col>
						</el-row>
					</div>
					<div style="margin-bottom: 10px; border: 0px solid red;">
						<el-row :gutter="24">
							...
							<el-col :span="18">
								<el-slider v-model="form.replenishRate" :step="1" :min="0" :max="form.burstCapacity"
									show-stops />
							</el-col>
						</el-row>
					</div>
					...
					<el-collapse accordion>
						<el-collapse-item>
							...
							<div><el-checkbox v-model="limiter.ipChecked"
									@change="handleLimiterChecked('ip')">启用</el-checkbox>
							</div>
						...
						<el-collapse-item title="URI限流">
							...
							<div><el-checkbox v-model="limiter.uriChecked"
									@change="handleLimiterChecked('uri')">启用</el-checkbox></div>
							<div>基于令牌桶算法，当访问网关路由URL的请求数量，超出限流规则的约束，则直接拒决请求。</div>
						</el-collapse-item>
						<el-collapse-item>
							<template #title>
								REQUESTID限流&nbsp;&nbsp;<i v-show="limiter.idChecked" class="header-icon el-icon-success"
									style="color: #34bfa3; font-size: 12pt;"></i>
							</template>
							<div><el-checkbox v-model="limiter.idChecked"
									@change="handleLimiterChecked('id')">启用</el-checkbox>
							</div>
			...
			<el-col :span="6">
				<el-card shadow="never" class="box-card">
					...
					<el-collapse accordion>
						<el-collapse-item>
							<template #title>
								HEADER验证&nbsp;&nbsp;<i v-show="access.headerChecked" class="header-icon el-icon-success"
									style="color: #34bfa3; font-size: 12pt;"></i>
							</template>
							<div>
							...
							<div style="margin-top: 10px;">
								<el-input size="large" placeholder="示例：Accept-Language: zh-CN,zh;q=0.9"
									v-model="form.accessHeader">
									<template #prepend>HEADER</template>
								</el-input>
							</div>
						</el-collapse-item>
						<el-collapse-item>
							...
							<div style="margin-top: 10px;">
								<el-input size="large" placeholder="示例：192.168.1.100,92.168.1.*"
						...
						<el-collapse-item>
							<template #title>
								参数验证&nbsp;&nbsp;<i v-show="access.parameterChecked" class="header-icon el-icon-success"
									style="color: #34bfa3; font-size: 12pt;"></i>
							</template>
						...
						<el-collapse-item title="Cookie验证">
							<template #title>
								Cookie验证&nbsp;&nbsp;<i v-show="access.cookieChecked" class="header-icon el-icon-success"
									style="color: #34bfa3; font-size: 12pt;"></i>
							</template>
